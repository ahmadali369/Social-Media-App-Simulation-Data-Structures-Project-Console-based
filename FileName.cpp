...
int main()
{
    ...
    ifstream inputFile("input.txt");  // Define file stream objects for reading and writing
    ofstream outputFile("output.txt");
    int opt;
    while (true)
    {
        Display_Menu();
        inputFile >> opt;// cin >> opt;
        inputFile.ignore();// cin.ignore(); // Clear the input buffer
        switch (opt)
        {
        case 1:
        {
            ...
            outputFile << "Enter User ID:         "; //cout << "Enter User ID:         ";
            inputFile >> User_Id;//cin >> User_Id;
            inputFile.ignore();// cin.ignore();
            outputFile << "Enter User Name:       "; //cout << "Enter User Name:       ";
            getline(inputFile, User_Name);  //getline(cin, User_Name);
            outputFile << "Enter Profile Details: "; //cout << "Enter Profile Details: ";
            getline(inputFile, Profile_Details);  //getline(cin, Profile_Details);
        ...
        case 2:
        {
            ...
            outputFile << "\t====================================================\n"; // cout << "\t====================================================\n";
            outputFile << "\t USER ID \t USER NAME \t PROFILE DETAILS  \n"; //cout << "\t USER ID \t USER NAME \t PROFILE DETAILS  \n";
            outputFile << "\t====================================================\n";// cout << "\t====================================================\n";

            if (userProfile != nullptr)
            {
                // cout << setw(12) << userProfile->User_Id << setw(17) << userProfile->User_Name << setw(22) << userProfile->Profile_Details << endl;
                outputFile << setw(12) << userProfile->User_Id << setw(17) << userProfile->User_Name << setw(22) << userProfile->Profile_Details << endl;
            }
            else
            {
                outputFile << "\t\t(_) USER PROFILE NOT FOUND (_) " << endl;//cout << "\t\t(_) USER PROFILE NOT FOUND (_) " << endl;
            }
            break;
        }
        case 3:
        {
            int User_Id;
            outputFile << "Enter User ID to remove: ";// cout << "Enter User ID to remove: ";
            inputFile >> User_Id; //cin >> User_Id;
            inputFile.ignore(); //cin.ignore();
        ...
        {
            int userId1, userId2;
            outputFile << "\tEnter User ID 1: ";// cout << "\tEnter User ID 1: ";
            inputFile >> userId1; //cin >> userId1;
            outputFile << "\tEnter User ID 2: ";// cout << "\tEnter User ID 2: ";
            inputFile >> userId2; //cin >> userId2;
        ...
        case 5:
        {
            ...
            outputFile << "\tEnter User ID 2: "; //cout << "\tEnter User ID 2: ";
            inputFile >> userId2;// cin >> userId2;
            / cout / outputFile << "\t\t USER  " << userId1 << " AND " << userId2 << " ARE FRIENDS ???   " << (obj_Friend_Network.Are_Friends(userId1, userId2) ? " * Yes * " : " !!! No !!!") << endl;
            break;
        }
        case 6:
        {
            ...
            outputFile << "\tEnter User ID:                  "; //cout << "\tEnter User ID:                ";
            inputFile >> User_Id; //cin >> User_Id;
            outputFile << "\tEnter Post ID:                  "; //cout << "\tEnter Post ID:                ";
            inputFile >> Post_Id; //cin >> Post_Id;
            inputFile.ignore(); //cin.ignore();
            outputFile << "\tEnter Post Content (string):  "; //cout << "\tEnter Post Content (string):  ";
            getline(inputFile, Post_Content); //getline(cin, Post_Content);
        ...
        {
            int targetUserId;
            outputFile << "Enter User ID to see posts: ";//cout << "Enter User ID to see posts: ";
            inputFile >> targetUserId;// cin >> targetUserId;
        ...
        case 8:
        {
            ...
            outputFile << "\tEnter Post ID:                  "; //cout << "\tEnter Post ID:                  ";
            inputFile >> Post_Id;// cin >> Post_Id;
            outputFile << "\tEnter Comment ID:               "; //cout << "\tEnter Comment ID:               ";
            inputFile >> commentId; //cin >> commentId;
            inputFile.ignore();
            outputFile << "\tEnter Comment Content (string): "; //cout << "\tEnter Comment Content (string): ";
            getline(inputFile, Comment_Content);
        ...
        case 10:
        {
            outputFile << "   ---- NOTIFICATIONS -----" << endl;//  cout << "   ---- NOTIFICATIONS -----" << endl;
            ...
            {
                outputFile << "Enter User ID: "; //cout << "Enter User ID: ";
                inputFile >> userId;// cin >> userId;

                if (userId == 1 || userId == 2)
                {
                    outputFile << "   WANNA PUSH NOTIFICATION? (y/n): "; //cout << "   WANNA PUSH NOTIFICATION? (y/n): ";
                    inputFile >> choice; //cin >> choice;
                    if (choice == 'y' || choice == 'Y')
                    {
                        string notificationMsg;
                        outputFile << "ENTER NOTIFICATION MESSAGE "; //cout << "ENTER NOTIFICATION MESSAGE ";
                        inputFile.ignore(); //cin.ignore();
                        getline(inputFile, notificationMsg); //getline(cin, notificationMsg);
                        obj_User_Notifications.Push_Notification(notificationMsg);
                    }

                    outputFile << "   WANNA SEE NOTIFICATIONS? (Y/N): ";// cout << "   WANNA SEE NOTIFICATIONS? (Y/N): ";
                    inputFile >> choice; //cin >> choice;
                    if (choice == 'y' || choice == 'Y')
                    {
                        outputFile << "____NOTIFICATIONS FOR USER___ " << userId << ":" << endl; //cout << "____NOTIFICATIONS FOR USER___ " << userId << ":" << endl;
                ...
                {
                    outputFile << "   (!!) NO NOTOFICATION FOR USER  " << userId << " (!!) " << endl;// cout << "   (!!) NO NOTOFICATION FOR USER  " << userId << " (!!) " << endl;
                }

                outputFile << "\n WANNA CONTINUE ? (Y/N): ";// cout << "\n WANNA CONTINUE ? (Y/N): ";
                inputFile >> choice; //cin >> choice;
        ...
        case 11:
            outputFile << "   #### EXITING ####" << endl; //cout << "   #### EXITING ####" << endl;
            return 0;
        default:
            outputFile << "   ..... INVALID ....." << endl;// cout << "   ..... INVALID ....." << endl;
            break;
        }
